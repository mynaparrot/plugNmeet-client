...
async function fetchRecordings(from = 0, limit = 5, order_by = 'DESC') {
...
async function downloadRecording(e) {
    ...
    if (res.status) {
        const url = window.PLUG_N_MEET_SERVER_URL + "/download/recording/" + res.token;
        window.open(url, "_blank");
    } else {
        alert(res.msg)
    }
...
function displayRecordings(recordings) {
    ...
    for (let i = 0; i < recordings.length; i++) {
        ...
        html += '<a href="#" class="download" data-recording="' + recording.record_id + '" onclick="downloadRecording(event)">Download</a>';
        if (IS_ADMIN) {
            html += '<a href="#" class="delete" data-recording="' + recording.record_id + '" onclick="deleteRecording(event)">Delete</a>';
        }
        html += '</div>';
