...
const reOrderWebcams = throttle(
  (speakers: IActiveSpeaker[], room: ReturnType<typeof getMediaServerConn>) => {
    ...
    for (let i = 0; i < speakers.length; i++) {
      ...
      const participant = room.getVideoSubscriberParticipant(speaker.userId);
...
const ActiveSpeakers = ({ activeSidePanel }) => {
  ...
  return (
    activeSpeakersElms && (
      <div
        className={`active-speakers-wrap flex items-center justify-center absolute top-0 start-0 pointer-events-none ${activeSidePanel ? 'md:w-[calc(100%-300px)] 3xl:w-[calc(100%-340px)]' : 'w-full'}`}
      >
        {activeSpeakersElms}
      </div>
    )
